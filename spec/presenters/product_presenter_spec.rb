...
describe ProductPresenter do
  ...
  it 'should show valid thumbnail with image in link' do
    @product = Factory(:product)
    @presenter = ProductPresenter.new(@product, view)
    @presenter.thumbnail_with_link.should include("http://placehold.it/200x200")
  end

  it 'should delegate missed methods to @template' do
    @product = Factory(:product)
    @presenter = ProductPresenter.new(@product, view)
    @presenter.request.should be_a(ActionController::TestRequest)
  end
end
